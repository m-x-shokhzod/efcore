...
/// <summary>
///     A convention that marks every complex property in the path to a property used by a key or an index as
///     required, since the value of such a property must be available in order to compute the key/index value.
/// </summary>
/// <remarks>
///     <para>
///         When the last referencing key or index is removed, the implicit non-nullable configuration applied
///         here is reverted as long as no other key or index keeps the chain in use and no explicit configuration
...
///     </para>
/// </remarks>
public class KeyOrIndexComplexPropertyConvention :
    IKeyAddedConvention,
...
{
    /// <summary>
    ///     Creates a new instance of <see cref="KeyOrIndexComplexPropertyConvention" />.
    /// </summary>
    /// <param name="dependencies">Parameter object containing dependencies for this convention.</param>
    public KeyOrIndexComplexPropertyConvention(ProviderConventionSetBuilderDependencies dependencies)
    ...
    public virtual void ProcessKeyAdded(
        ...
        => MarkChainAsRequired(keyBuilder.Metadata.Properties.Cast<IConventionPropertyBase>().ToList());

    /// <inheritdoc />
    public virtual void ProcessIndexAdded(
        IConventionIndexBuilder indexBuilder,
        IConventionContext<IConventionIndexBuilder> context)
        => MarkChainAsRequired(indexBuilder.Metadata.Properties);
    ...
    public virtual void ProcessKeyRemoved(
        ...
        => TryRevertChain(key.Properties.Cast<IConventionPropertyBase>().ToList());
    ...
    public virtual void ProcessIndexRemoved(
        ...
        => TryRevertChain(index.Properties);

    private static void MarkChainAsRequired(IReadOnlyList<IConventionPropertyBase> properties)
    {
        foreach (var property in properties)
        {
            ...
            // TODO: Use layering #15898 - the implicit "required because referenced by a key/index" state
    ...
    private static void TryRevertChain(IReadOnlyList<IConventionPropertyBase> properties)
    {
        foreach (var property in properties)
        {
            if (property.DeclaringType is not IConventionComplexType)
            ...
            foreach (var complexProperty in EnumerateChain(property))
            {
                ...
                // that was applied because of a key/index from one applied for any other reason, so we
                // only revert when no key or index in the entity references any property declared on the
    ...
    private static bool IsStillReferenced(IConventionComplexProperty complexProperty)
    {
        ...
        {
            if (index.Properties.Any(p => coveredTypes.Contains(p.DeclaringType)))
            {
                return true;
            }
        }
