    ...
    bool threadSafetyChecksEnabled)
{
    ...
    ///     <see cref="NewExpression" /> constructor arguments), boxed <see cref="RelationalTypeMapping.ReadValue" />
    ///     is used instead, since boxing is unavoidable for <see cref="ConstructorInvoker" />.
    /// </summary>
        ...
        bool isTracking)
    {
        ...
        switch (shaperExpression)
        {
            case ProjectionBindingExpression scalarProjection:
            {
                ...
                {
                    // Typed path: use GetFieldValue<T> to avoid boxing (top-level scalar projection).
                    return nullable
                        ? (qc, reader, rc, coord) => reader.IsDBNull(columnIndex) ? default : reader.GetFieldValue<T>(columnIndex)
                        : (qc, reader, rc, coord) => reader.GetFieldValue<T>(columnIndex);
                }

                // Boxed path: use ReadValue for correct type coercion when T is object
                // (e.g. anonymous type constructor args where boxing is unavoidable).
                var typeMapping = select.Projection[columnIndex].Expression.TypeMapping!;
                return nullable
                    ? (qc, reader, rc, coord) => reader.IsDBNull(columnIndex) ? default : (T?)typeMapping.ReadValue(reader, columnIndex)
                    : (qc, reader, rc, coord) => (T?)typeMapping.ReadValue(reader, columnIndex)!;
            }
            ...
            case NewExpression newExpression:
            {
                ...
                // Anonymous types are always reference types, so Unsafe.As from Func<..., object?> to
                // Func<..., T?> is a safe reinterpretation of the delegate's return type.
                Func<QueryContext, DbDataReader, ResultContext, SingleQueryResultCoordinator, object?> boxed = m.Length switch
                {
                    0 => (queryCtx, reader, rc, coord)
                        => invoker.Invoke(),
                    1 => (queryCtx, reader, rc, coord)
                        => invoker.Invoke(m[0](queryCtx, reader, rc, coord)),
                    2 => (queryCtx, reader, rc, coord)
                        => invoker.Invoke(m[0](queryCtx, reader, rc, coord), m[1](queryCtx, reader, rc, coord)),
                    ...
                    _ => (queryCtx, reader, rc, coord) =>
                    {
                        ...
                        return invoker.Invoke(args.AsSpan());
                    }
                };

                return Unsafe.As<Func<QueryContext, DbDataReader, ResultContext, SingleQueryResultCoordinator, T?>>(boxed);
            }
            ...
            case IncludeExpression:
            {
                ...
                return Unsafe.As<Func<QueryContext, DbDataReader, ResultContext, SingleQueryResultCoordinator, T?>>(
                    (Func<QueryContext, DbDataReader, ResultContext, SingleQueryResultCoordinator, object?>)entityMaterializer.Materialize);
            }
