...
#pragma warning disable EF1001 // Internal EF Core API usage.
            => index.GetIncludeProperties()!
                .Select(p => RelationalModel.FindReadOnlyPropertyByPath(index.DeclaringEntityType, p)!.GetColumnName(storeObject))
                .SequenceEqual(
                    duplicateIndex.GetIncludeProperties()!.Select(p
                        => RelationalModel.FindReadOnlyPropertyByPath(duplicateIndex.DeclaringEntityType, p)!.GetColumnName(storeObject)));
...
#pragma warning disable EF1001 // Internal EF Core API usage.
                index.GetIncludeProperties()!.Select(p
                    => RelationalModel.FindReadOnlyPropertyByPath(index.DeclaringEntityType, p)?.GetColumnName(storeObject)))
#pragma warning restore EF1001 // Internal EF Core API usage.
            + "'}";
}
